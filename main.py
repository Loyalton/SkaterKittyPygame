...
while running:
    ...
    if gameActive == True:
        ...
        # # Update counter for spawning
        # specialItemSpawnCounter += dt

        # if spawnSpecialItemBool:
        #     # Choose a random item to spawn
        #     specialItemSpawnInt = random.randint(0, len(spawnSpecialItemList) - 1)
        #     item = spawnSpecialItemList[specialItemSpawnInt]

        #     if item.rect.right > 0:  # Ensure item is on-screen
        #         item.update()
        #         spawnSpecialItemBool = False
        # else:
        #     # Update currently active item
        #     for item in spawnSpecialItemList:
        #         if item.spawnItem:
        #             item.update()

        #     # Reset spawn if conditions are met
        #     if specialItemSpawnCounter >= 500:  # Adjust for timing
        #         spawnSpecialItemBool = True
        #         specialItemSpawnCounter = 0
